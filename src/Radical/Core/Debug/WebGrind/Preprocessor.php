...
class Preprocessor
{
	...
	static function parse($inFile, $outFile)
	{
		...
		while(!feof($in)){
			...
			if(substr_compare($line,'fl=',0,3)===0){
			...
			} else if(substr_compare($line,'cfn=',0,4)===0) {
				...
				if(!isset($functions[$calledFunctionName]['calledFromInformation'][($f = $function.':'.$lnr)]))
					$functions[$calledFunctionName]['calledFromInformation'][$f] = array('functionNr'=>$functions[$function]['nr'],'line'=>$lnr,'callCount'=>0,'summedCallCost'=>0);
				
				$functions[$calledFunctionName]['calledFromInformation'][$f]['callCount']++;
				$functions[$calledFunctionName]['calledFromInformation'][$f]['summedCallCost'] += $cost;

				if(!isset($functions[$function]['subCallInformation'][($f = $calledFunctionName.':'.$lnr)]))
					$functions[$function]['subCallInformation'][$f] = array('functionNr'=>$functions[$calledFunctionName]['nr'],'line'=>$lnr,'callCount'=>0,'summedCallCost'=>0);
				
				$functions[$function]['subCallInformation'][$f]['callCount']++;
				$functions[$function]['subCallInformation'][$f]['summedCallCost'] += $cost;
